...
import kr.ac.inu.deepect.arnavigation.sensor.DeviceLocationChanged;
import kr.ac.inu.deepect.arnavigation.sensor.DeviceOrientation;
...
public class LocationScene {
    ...
    public static ArrayList<LocationMarker> mLocationMarkers = new ArrayList<>();
    public static void mLocationMarkersClear() {
    ...
    private int anchorRefreshInterval = 1000 * 1; // 1 seconds에 한 번씩 다시 그림.
    ...
    private void test() {
        ...
        float bearing = (float) ARUtils.bearing(
                48.31244200607186,
                2.1290194140624408,
                48.33577350525661,
                2.073057805175722);
...
//        }
    ...
    private void refreshAnchorsIfRequired(Frame frame) {
        ...
        for (int i = 0; i < mLocationMarkers.size(); i++) {
            try {
                ...
                int markerDistance;
                float bearing, markerBearing;
                if (marker.isAtCameraPosition()) {
                    markerDistance = 0;
                    bearing = 0;
                    markerBearing = 0;
                }
                else {
                    markerDistance = (int) Math.round(
                            ARUtils.distance(
                                    marker.latitude,
                    ...
                    bearing = (float) ARUtils.bearing(
                            deviceLocation.currentBestLocation.getLatitude(),
                            deviceLocation.currentBestLocation.getLongitude(),
                            marker.latitude,
                            marker.longitude);

                    markerBearing = bearing - deviceOrientation.getOrientation();
                }
     ...
     */
    // 일반적으로 onResume() 메소드 내에서 호출되게 된다.
    public void resume() {
        deviceOrientation.resume();
